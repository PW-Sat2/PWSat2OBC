...
void BSP_UART_txBuffer(USART_TypeDef* usart, const uint8_t* buff, uint16_t len, bool wait); ///< Transmit data buffer over specified UART.
...
template <std::size_t BufferSize> __attribute__((format(printf, 2, 3))) void BSP_UART_Printf(USART_TypeDef* usart, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);

    char buffer[BufferSize] = {0};
    auto length = vsprintf(buffer, fmt, args);

    BSP_UART_txBuffer(usart, reinterpret_cast<const uint8_t*>(buffer), length, true);
