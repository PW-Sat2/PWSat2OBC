...
#define BOOT_getOffsetEntry(index)                                                                                                         \
    (BOOT_TABLE_OFFSET_ENTRY1 +                                                                                                            \
        (BOOT_TABLE_ENTRY_SIZE * (index - 1))) ///< Returns the offset from base to the entry in the boot table at the specified index
#define BOOT_getOffsetLen(index)                                                                                                           \
    (BOOT_TABLE_OFFSET_ENTRY1 + (BOOT_TABLE_ENTRY_SIZE * (index - 1)) +                                                                    \
        BOOT_TABLE_OFFSET_ENTRYLENGTH) ///< Returns the offset from base to the entry's length in the boot table at the specified index
#define BOOT_getOffsetCRC(index)                                                                                                           \
    (BOOT_TABLE_OFFSET_ENTRY1 + (BOOT_TABLE_ENTRY_SIZE * (index - 1)) +                                                                    \
        BOOT_TABLE_OFFSET_ENTRYCRC) ///< Returns the offset from base to the entry's CRC value in the boot table at the specified index
#define BOOT_getOffsetValid(index)                                                                                                         \
    (BOOT_TABLE_OFFSET_ENTRY1 + (BOOT_TABLE_ENTRY_SIZE * (index - 1)) +                                                                    \
        BOOT_TABLE_OFFSET_ENTRYVALID) ///< Returns the offset from base to the entry's valid flag in the boot table at the specified index
#define BOOT_getOffsetDescription(index)                                                                                                   \
    (BOOT_TABLE_OFFSET_ENTRY1 + (BOOT_TABLE_ENTRY_SIZE * (index - 1)) + BOOT_TABLE_OFFSET_ENTRYDESRCIPTION) ///< Returns the offset from
                                                                                                            ///base to the entry's
                                                                                                            ///description in the boot table
                                                                                                            ///at the specified index
#define BOOT_getOffsetProgram(index)                                                                                                       \
    (BOOT_TABLE_OFFSET_ENTRY1 + (BOOT_TABLE_ENTRY_SIZE * (index - 1)) +                                                                    \
        BOOT_TABLE_OFFSET_ENTRYPROGRAM) ///< Returns the offset from base to the entry's program code in the boot table at the specified
                                        ///index
/** @} */

typedef enum { downloadSuccessful, indexNotValid, entryNotValid, entryCRCMismatch, downloadCRCMismatch } BOOT_DownloadResult_Typedef;
...
void BOOT_programEntry(
    uint8_t index, uint32_t offset, uint8_t* buffer, uint32_t len); ///< Program new entry in boot table at specified index.
...
BOOT_DownloadResult_Typedef BOOT_tryDownloadEntryToApplicationSpace(
    uint8_t index);                       ///< Download an entry from boot table into MCU application space.
