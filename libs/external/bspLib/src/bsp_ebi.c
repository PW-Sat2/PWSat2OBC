...
uint8_t BSP_EBI_enableSRAM (BSP_EBI_SRAMSelect_TypeDef module)
{
	...
	switch (module)
	{
	/*Power switch to each SRAM: This switch has over-current protection and will shut off if the constant current condition persists after 10ms.
...
#endif
			// SRAM1 is now connected and on. Test by writing and reading to the SRAMs. Note that this test will pass if SRAM2 is already turned on even tough SRAM1 might not be working.
			*(uint8_t*)(BSP_EBI_SRAM1_BASE) = TestValue;	// Write test value to the first SRAM address
			Delay(1);
			*(uint8_t*)(BSP_EBI_SRAM1_BASE+1) = TestValue+1;	// Write a value to nexxt address
			Delay(1);
			ReadValue = *(uint8_t*)(BSP_EBI_SRAM1_BASE);	//Read test value from the first address in SRAM
			if(ReadValue != TestValue)
			{
				result = 0;							// If the wrong value is read something went wrong and the SRAM was not successfully turned on and connected to the bus
...
#endif
			// SRAM2 is now connected and on. Test that we can write and read to the SRAMs. Note that this test will pass if the SRAM1 is already turned on.
			*(uint8_t*)(BSP_EBI_SRAM1_BASE) = TestValue;		// Write test value to the first SRAM address
			Delay(1);
			*(uint8_t*)(BSP_EBI_SRAM1_BASE+1) = TestValue+1;	// Write a value to nexxt address
			Delay(1);
			ReadValue = *(uint8_t*)(BSP_EBI_SRAM1_BASE);
			if(ReadValue != TestValue)
			{
				result = 0;							// If the wrong value is read something went wrong and the SRAM was not successfully turned on and connected to the bus
