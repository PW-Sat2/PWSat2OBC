...
void BSP_UNIX_Init()
{
	...
	if ( !(resetcause & RMU_RSTCAUSE_BUBODVDDDREG) && ( (resetcause & RMU_RSTCAUSE_SYSREQRST) || (resetcause & RMU_RSTCAUSE_LOCKUPRST) ) ) //If the reset cause was not Backup Brown-Out Detector on BUVIN, but caused by a system request or a lockup
	{
	...
	//if ( !(resetcause & RMU_RSTCAUSE_BUBODBUVIN) && ( (resetcause & RMU_RSTCAUSE_SYSREQRST) || (resetcause & RMU_RSTCAUSE_LOCKUPRST) ) ) //If the reset cause was not Backup Brown-Out Detector on BUVIN, but caused by a system request or a lockup (use if backup power supply is available)
	if ( !(resetcause & RMU_RSTCAUSE_BUBODVDDDREG) && ( (resetcause & RMU_RSTCAUSE_SYSREQRST) || (resetcause & RMU_RSTCAUSE_LOCKUPRST) ) ) //If the reset cause was not Backup Brown-Out Detector on BUVIN, but caused by a system request or a lockup
	{
		unixEpoch = BURTC_RetRegGet(0); // Read the unixEpoch from the Backup RTC Retention register
		return;
...
void initBackupPowerDomain(void)
{
	...
	em4Init.osc = emuEM4Osc_LFXO;			// Use LFXO oscillator
	...
	bupdInit.enable = true;					// Enable backup power domain,and release reset, enable BU_VIN pin [enter on BOD, enable BU_VIN pin, release BU reset]

	EMU_EM4Lock( false ); 					// Unlock configuration

	EMU_BUPDInit( &bupdInit );				// Configure Backup Power Domain settings

#if defined(CubeCompV3B)					// BU_VIN pin only available on CubeComputer V3B. User can access this pin on the Piggyback header (P600) pin 27. A suitable power source must be connected to pin to supply power to Backup power domain.
	EMU_BUPinEnable(true);					// Enable BU_VIN support (BUVINPEN in EMU_ROUTE is cleared)
#endif
#if defined(CubeCompV3)						// BU_Vin pin is not available on CubeComputer V3A. The Backup power domain is only powered by the normal VDDReg received by the MCU.
	...
	RMU_ResetControl( rmuResetBU, true ); 	// Enable backup power domain,and release reset
...
void initBackupRTC(void)
{
	...
	burtcInit.debugRun = false;					// If true, counter will keep running under debug halt, flase so counter will stop under debug halt
