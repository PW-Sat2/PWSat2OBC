...
uint32_t dataScrubPieceSize = 4; // BSP_EBI_SRAM_SIZE;
...
void BSP_SEE_checkMemory(void)
{
    ...
    if (sram1Latched)
        BSP_SEE_tryLatchupRecovery(bspSeeSram1, latchupRetryCount);

    if (sram2Latched)
        BSP_SEE_tryLatchupRecovery(bspSeeSram2, latchupRetryCount);
}
...
uint8_t BSP_SEE_tryLatchupRecovery(BSP_See_SRAMSelect_TypeDef sram, uint8_t retryMax)
{
    ...
    {
        recovered = BSP_EBI_enableSRAM(static_cast<BSP_EBI_SRAMSelect_TypeDef>(sram));
    }
...
void BSP_SEE_doPiecewiseDataScrub(void)
{
    ...
    char buf[40] = {0};
    sprintf(buf, "\nS %d D %d M %d", dataScrubCurResult.singleErrors, dataScrubCurResult.doubleErrors, dataScrubCurResult.multiErrors);
...
void checkAndLogErrors(void)
{
    ...
    //    char buf[30] = {0};
    //    sprintf(buf, "\nError: %X", errors);
    //    BSP_UART_txBuffer(BSP_UART_DEBUG, (uint8_t*)buf, 10, true);
