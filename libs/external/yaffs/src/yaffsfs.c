...
struct yaffs_obj* yaffsfs_FindObject(
    struct yaffs_obj* relativeDirectory, const YCHAR* path, int symDepth, int getEquiv, struct yaffs_obj** dirOut, int* notDir, int* loop);

static void yaffsfs_RemoveObjectCallback(struct yaffs_obj* obj);
static yaffs_DIR* yaffsfs_opendir_reldir_no_lock(struct yaffs_obj* reldir, const YCHAR* dirname);
...
static int yaffsfs_alt_dir_path(const YCHAR* path, YCHAR** ret_path)
{
    ...
    *ret_path = NULL;
    ...
    if (path_length > 0 && yaffsfs_IsPathDivider(path[path_length - 1]))
    {
        ...
        for (i = path_length - 1; i >= 0 && yaffsfs_IsPathDivider(alt_path[i]); i--)
            alt_path[i] = (YCHAR)0;
    }
    *ret_path = alt_path;
    return 0;
...
static struct yaffs_dev* yaffsfs_FindDevice(const YCHAR* path, YCHAR** restOfPath)
{
    ...
    list_for_each(cfg, &yaffsfs_deviceList)
    {
        ...
        while (matching && *p && *leftOver)
        {
            ...
            while (matching && *p && !yaffsfs_IsPathDivider(*p) && *leftOver && !yaffsfs_IsPathDivider(*leftOver))
            {
            ...
            if ((*p && !yaffsfs_IsPathDivider(*p)) || (*leftOver && !yaffsfs_IsPathDivider(*leftOver)))
                matching = 0;
            else
            {
                while (yaffsfs_IsPathDivider(*p))
 ...
 */
static struct yaffs_obj* yaffsfs_DoFindDirectory(
    struct yaffs_obj* startDir, const YCHAR* path, YCHAR** name, int symDepth, int* notDir, int* loop)
{
...
static struct yaffs_obj* yaffsfs_FindDirectory(
    struct yaffs_obj* relDir, const YCHAR* path, YCHAR** name, int symDepth, int* notDir, int* loop)
{
...
int yaffs_open_sharing_reldir(struct yaffs_obj* reldir, const YCHAR* path, int oflag, int mode, int sharing)
{
    ...
    else
    {
        ...
        if (obj)
        {
            ...
            is_dir = (obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY);

            /* A directory can't be opened except for read */
            if (is_dir && (writeRequested || !readRequested || rwflags != O_RDONLY))
            {
                openDenied = __LINE__;
            ...
            if (!openDenied)
            {
                ...
                {
                    fdx = &yaffsfs_fd[i];
                    if (fdx->handleCount > 0 && fdx->inodeId >= 0 && yaffsfs_inode[fdx->inodeId].iObj == obj)
                    {
                        if (!fdx->shareRead)
                            sharedReadAllowed = 0;
                        if (!fdx->shareWrite)
                ...
                if ((!sharedReadAllowed && readRequested) || (!shareRead && alreadyReading) || (!sharedWriteAllowed && writeRequested) ||
...
{
    return yaffs_open_sharing_reldir(ROOT_DIR(dev), path, oflag, mode, sharing);
}
...
{
    return yaffs_open_sharing_reldir(reldir, path, oflag, mode, YAFFS_SHARE_READ | YAFFS_SHARE_WRITE);
}

int yaffs_open_reldev(struct yaffs_dev* dev, const YCHAR* path, int oflag, int mode)
{
    return yaffs_open_sharing_reldir(ROOT_DIR(dev), path, oflag, mode, YAFFS_SHARE_READ | YAFFS_SHARE_WRITE);
}
...
static int yaffsfs_do_read(int handle, void* vbuf, unsigned int nbyte, int isPread, Y_LOFF_T offset)
{
    struct yaffsfs_FileDes* fd = NULL;
    struct yaffs_obj* obj = NULL;
    Y_LOFF_T pos = 0;
    ...
    else
    {
        ...
        if (pos < 0 || pos > YAFFS_MAX_FILE_SIZE || nbyte > YAFFS_MAX_FILE_SIZE || endPos < 0 || endPos > YAFFS_MAX_FILE_SIZE)
        {
            totalRead = -1;
            nbyte = 0;
        }
...
static int yaffsfs_do_write(int handle, const void* vbuf, unsigned int nbyte, int isPwrite, Y_LOFF_T offset)
{
    struct yaffsfs_FileDes* fd = NULL;
    struct yaffs_obj* obj = NULL;
    Y_LOFF_T pos = 0;
...
static int yaffsfs_DoUnlink_reldir(struct yaffs_obj* reldir, const YCHAR* path, int isDirectory)
{
    struct yaffs_obj* dir = NULL;
    struct yaffs_obj* obj = NULL;
    YCHAR* name;
...
int yaffs_funlink(int fd)
{
    ...
    if (!obj)
        ...
        yaffsfs_SetError(-EROFS);
    else if (obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY && !(list_empty(&obj->variant.dir_variant.children)))
        yaffsfs_SetError(-ENOTEMPTY);
...
{
    if (obj && obj->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY && newobj && newobj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
        return 1;
    else
        return 0;
}
...
int yaffs_rename_reldir(struct yaffs_obj* reldir, const YCHAR* oldPath, const YCHAR* newPath)
{
    ...
    if (yaffsfs_CheckMemRegion(oldPath, 0, 0) < 0 || yaffsfs_CheckMemRegion(newPath, 0, 0) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
    ...
    olddir = yaffsfs_FindDirectory(reldir, oldPath, &oldname, 0, &notOldDir, &oldLoop);
    newdir = yaffsfs_FindDirectory(reldir, newPath, &newname, 0, &notNewDir, &newLoop);
...
int yaffs_rename_reldev(struct yaffs_dev* dev, const YCHAR* oldPath, const YCHAR* newPath)
{
...
static int yaffsfs_DoStatOrLStat_reldir(struct yaffs_obj* reldir, const YCHAR* path, struct yaffs_stat* buf, int doLStat)
{
    struct yaffs_obj* obj = NULL;
    struct yaffs_obj* dir = NULL;
    int retVal = -1;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(buf, sizeof(*buf), 1) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
static int yaffs_do_setxattr_reldir(
    struct yaffs_obj* reldir, const YCHAR* path, const char* name, const void* data, int size, int flags, int follow)
{
    struct yaffs_obj* obj;
    struct yaffs_obj* dir;
    int notDir = 0;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(name, 0, 0) < 0 || yaffsfs_CheckMemRegion(data, size, 0) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
int yaffs_setxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, const void* data, int size, int flags)
{
    return yaffs_do_setxattr_reldir(reldir, path, name, data, size, flags, 1);
}

int yaffs_setxattr_reldev(struct yaffs_dev* dev, const YCHAR* path, const char* name, const void* data, int size, int flags)
{
...
int yaffs_lsetxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, const void* data, int size, int flags)
{
    return yaffs_do_setxattr_reldir(reldir, path, name, data, size, flags, 0);
}

int yaffs_lsetxattr_reldev(struct yaffs_dev* dev, const YCHAR* path, const char* name, const void* data, int size, int flags)
{
...
int yaffs_fsetxattr(int fd, const char* name, const void* data, int size, int flags)
{
    ...
    if (yaffsfs_CheckMemRegion(name, 0, 0) < 0 || yaffsfs_CheckMemRegion(data, size, 0) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
static int yaffs_do_getxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, void* data, int size, int follow)
{
    struct yaffs_obj* obj;
    struct yaffs_obj* dir;
    int retVal = -1;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(name, 0, 0) < 0 || yaffsfs_CheckMemRegion(data, size, 1) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
int yaffs_getxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, void* data, int size)
{
...
int yaffs_lgetxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, void* data, int size)
{
...
int yaffs_fgetxattr(int fd, const char* name, void* data, int size)
{
    ...
    if (yaffsfs_CheckMemRegion(name, 0, 0) < 0 || yaffsfs_CheckMemRegion(data, size, 1) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
static int yaffs_do_listxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, char* data, int size, int follow)
{
    struct yaffs_obj* obj = NULL;
    struct yaffs_obj* dir = NULL;
    int retVal = -1;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(data, size, 1) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
int yaffs_listxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, char* data, int size)
{
...
int yaffs_llistxattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, char* data, int size)
{
...
static int yaffs_do_removexattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name, int follow)
{
    struct yaffs_obj* obj = NULL;
    struct yaffs_obj* dir = NULL;
    int notDir = 0;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(name, 0, 0) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
int yaffs_removexattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name)
{
...
int yaffs_lremovexattr_reldir(struct yaffs_obj* reldir, const YCHAR* path, const char* name)
{
...
int yaffs_mount_common(struct yaffs_dev* dev, const YCHAR* path, int read_only, int skip_checkpt)
{
    int retVal = -1;
    int result = YAFFS_FAIL;
    ...
    if (!dev)
    {
        ...
        yaffs_tracef(YAFFS_TRACE_MOUNT, "yaffs: Mounting %s", path);
...
static int yaffs_sync_common(struct yaffs_dev* dev, const YCHAR* path, int do_checkpt)
{
    int retVal = -1;
    YCHAR* dummy;
...
static int yaffsfs_bg_gc_common(struct yaffs_dev* dev, const YCHAR* path, int urgency)
{
    int retVal = -1;
    YCHAR* dummy;
...
int yaffs_remount_common(struct yaffs_dev* dev, const YCHAR* path, int force, int read_only)
{
    int retVal = -1;
...
int yaffs_format_common(struct yaffs_dev* dev, const YCHAR* path, int unmount_flag, int force_unmount_flag, int remount_flag)
{
    int retVal = 0;
    int result;
...
int yaffs_format_reldev(struct yaffs_dev* dev, int unmount_flag, int force_unmount_flag, int remount_flag)
{
    return yaffs_format_common(dev, NULL, unmount_flag, force_unmount_flag, remount_flag);
}

int yaffs_format(const YCHAR* path, int unmount_flag, int force_unmount_flag, int remount_flag)
{
...
{
    if (dsc && dsc->dirObj && dsc->dirObj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
    {
        dsc->offset = 0;
        ...
        if (list_empty(&dsc->dirObj->variant.dir_variant.children))
            ...
            dsc->nextReturn = list_entry(dsc->dirObj->variant.dir_variant.children.next, struct yaffs_obj, siblings);
    }
    else
    {
...
static void yaffsfs_DirAdvance(struct yaffsfs_DirSearchContext* dsc)
{
    if (dsc && dsc->dirObj && dsc->dirObj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
    {
        if (dsc->nextReturn == NULL || list_empty(&dsc->dirObj->variant.dir_variant.children))
            dsc->nextReturn = NULL;
        else
        {
...
static void yaffsfs_RemoveObjectCallback(struct yaffs_obj* obj)
{
    ...
    list_for_each(i, &search_contexts)
    {
        if (i)
        {
            dsc = list_entry(i, struct yaffsfs_DirSearchContext, others);
...
int yaffs_symlink_reldir(struct yaffs_obj* reldir, const YCHAR* oldpath, const YCHAR* newpath)
{
    struct yaffs_obj* parent = NULL;
    struct yaffs_obj* obj;
    YCHAR* name;
    ...
    else if (parent)
    {
        ...
        if (obj)
            retVal = 0;
        else if (yaffsfs_FindObject(reldir, newpath, 0, 0, NULL, NULL, NULL))
            yaffsfs_SetError(-EEXIST);
        else
            yaffsfs_SetError(-ENOSPC);
    }
...
int yaffs_readlink_reldir(struct yaffs_obj* reldir, const YCHAR* path, YCHAR* buf, int bufsiz)
{
    struct yaffs_obj* obj = NULL;
    struct yaffs_obj* dir = NULL;
    int retVal = -1;
    ...
    if (yaffsfs_CheckMemRegion(path, 0, 0) < 0 || yaffsfs_CheckMemRegion(buf, bufsiz, 1) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
...
int yaffs_link_reldir(struct yaffs_obj* reldir, const YCHAR* oldpath, const YCHAR* linkpath)
{
    ...
    if (yaffsfs_CheckMemRegion(oldpath, 0, 0) < 0 || yaffsfs_CheckMemRegion(linkpath, 0, 0) < 0)
    {
        yaffsfs_SetError(-EFAULT);
        return -1;
    }
    ...
    obj = yaffsfs_FindObject(reldir, oldpath, 0, 1, &obj_dir, &notDirObj, &objLoop);
    lnk = yaffsfs_FindObject(reldir, linkpath, 0, 0, NULL, NULL, NULL);
    lnk_dir = yaffsfs_FindDirectory(reldir, linkpath, &newname, 0, &notDirLnk, &lnkLoop);
...
int yaffs_mknod_reldir(struct yaffs_obj* reldir, const YCHAR* pathname, mode_t mode, dev_t dev_val)
{
    (void)pathname;
    (void)mode;
    (void)dev_val;
