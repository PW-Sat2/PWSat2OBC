...
#define YAFFS_MAX_FILE_SIZE ((sizeof(Y_LOFF_T) < 8) ? YAFFS_MAX_FILE_SIZE_32 : (0x800000000LL - 1))
...
int yaffs_mknod_reldir(struct yaffs_obj* reldir, const YCHAR* pathname, mode_t mode, dev_t dev);
...
int yaffs_symlink_reldev(struct yaffs_dev* dev, const YCHAR* oldpath, const YCHAR* newpath);
int yaffs_readlink_reldev(struct yaffs_dev* dev, const YCHAR* path, YCHAR* buf, int bufsiz);
int yaffs_link_reldev(struct yaffs_dev* dev, const YCHAR* oldpath, const YCHAR* linkpath);
