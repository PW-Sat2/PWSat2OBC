...
class Frame
{
  public:
    ...
    /**
     * @brief ctor.
     * @param[in] dopplerLevel Frame doppler frequency.
     * @param[in] rssiLevel Received Signal Strength Indicator (RSSI) at the frame reception time.
     * @param[in] fullSize Full frame size.
     * @param[in] data span that contains entire received frame
     */
    Frame(std::uint16_t dopplerLevel, std::uint16_t rssiLevel, std::uint16_t fullSize, gsl::span<std::uint8_t> data);
