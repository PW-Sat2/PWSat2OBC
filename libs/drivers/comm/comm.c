...
static bool SendCommandWithResponse(CommObject* object, CommAddress address, uint8_t command, uint8_t* outBuffer, uint8_t outBufferSize)
{
    const I2CResult result =
        object->low->System->WriteRead(object->low->System, address, &command, sizeof(command), outBuffer, outBufferSize);
    const bool status = (result == I2CResultOK);
    if (!status)
    {
        LOGF(LOG_LEVEL_ERROR, "[comm] Unable to send command %d to %d, Reason: %d", command, address, result);
...
bool CommSendFrame(CommObject* comm, uint8_t* data, uint8_t length)
{
    ...
    const bool status =
        (comm->low->System->WriteRead(comm->low->System, CommTransmitter, cmd, length + 1, &remainingBufferSize, 1) == I2CResultOK);
    if (!status)
    {
...
bool CommSetBeacon(CommObject* comm, const CommBeacon* beaconData)
{
    ...
    return comm->low->System->Write(comm->low->System, CommTransmitter, buffer, WriterGetDataLength(&writer)) == I2CResultOK;
}
...
bool CommSetTransmitterStateWhenIdle(CommObject* comm, CommTransmitterIdleState requestedState)
{
    ...
    return comm->low->System->Write(comm->low->System, CommTransmitter, buffer, COUNT_OF(buffer)) == I2CResultOK;
}
...
bool CommGetTransmitterState(CommObject* comm, CommTransmitterState* state)
{
    ...
    uint8_t response;
    const bool status = (comm->low->System->WriteRead(
                             comm->low->System, CommTransmitter, &command, sizeof(command), &response, sizeof(response)) == I2CResultOK);
    if (!status)
    {
        return false;
    }
