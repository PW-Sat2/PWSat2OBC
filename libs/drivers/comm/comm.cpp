...
void CommObject::CommTask(void* param)
{
    ...
    auto anyFrameReceived = comm->PollHardware();
    for (;;)
    {
        auto loopDelay = anyFrameReceived ? 0s : 1s;

        const OSEventBits result = comm->_pollingTaskFlags.WaitAny(TaskFlagPauseRequest, true, loopDelay);
