...
namespace mission
{
    ...
    void TimeTask::CorrectTime(SystemState& state)
    {
        ...
        {
            ReaderLock lock(state.PersistentState.Lock, InfiniteTimeout);
            if (!lock())
            {
            ...
            newTime = PerformTimeCorrection(time.Value,
                currentRtcTime,
        ...
        {
            WriterLock lock(state.PersistentState.Lock, InfiniteTimeout);
            if (!lock())
            {
                LOG(LOG_LEVEL_ERROR, "[Time] Unable to acquire PersistentState lock.");
                return;
            }
