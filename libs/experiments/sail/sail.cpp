    ...
    namespace sail
    {
        ...
        experiments::StartResult SailExperiment::Start()
        {
            do
            {
                ...
                telemetrySequence = 0;
                _file.SetOnFlush(ExperimentFile::OnFlushDelegate::make_delegate<SailExperiment, &SailExperiment::SendExperimentData>(this));
        ...
        void SailExperiment::SendExperimentData(const gsl::span<uint8_t>& data)
        {
            LOGF(LOG_LEVEL_INFO, "[exp_sail] Sending experiment data, sequence %lu", telemetrySequence);

            DownlinkFrame message(DownlinkAPID::SailExperiment, telemetrySequence++);
