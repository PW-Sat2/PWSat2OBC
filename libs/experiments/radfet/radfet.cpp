...
namespace experiment
{
    namespace radfet
    {
        ...
        void RadFETExperiment::Stop(experiments::IterationResult /* lastResult */)
        {
            ...
            {
                LOG(LOG_LEVEL_ERROR, "Can't tunr off power to Sans");
                return;
            }
            ...
            if (!SaveEndTelemetry(this->dataSet, telemetry, currentTime.Value))
            {
            ...
            ExperimentFile& file, PayloadTelemetry::Radfet& telemetry, std::chrono::milliseconds timestamp)
        {
            ...
            if (!SaveRadFETRegisters(file, telemetry, ExperimentFile::PID::RadFETStart))
            {
            ...
            ExperimentFile& file, PayloadTelemetry::Radfet& telemetry, std::chrono::milliseconds timestamp)
        {
            ...
            if (!SaveRadFETRegisters(file, telemetry, ExperimentFile::PID::RadFETEnd))
            {
                LOG(LOG_LEVEL_ERROR, "Can't save RadFET registers");
                return false;
            }
            ...
            std::chrono::milliseconds timestamp)
        {
            ...
            if (!SaveRadFETRegisters(file, radfetTelemetry, ExperimentFile::PID::RadFETEnd))
            {
                LOG(LOG_LEVEL_ERROR, "Can't save RadFET registers");
                return false;
            }
