...
namespace std
{
    namespace chrono
    {
        template <typename _Rep, typename _Period = ratio<1>> struct frequency;
        ...
        template <typename _ToFreq, typename _CF, typename _CR, bool _NumIsOne = false, bool _DenIsOne = false> struct __frequency_cast_impl
        {
            template <typename _Rep, typename _Period> static constexpr _ToFreq __cast(const frequency<_Rep, _Period>& __d)
            {
                ...
                return _ToFreq(
                    static_cast<__to_rep>(static_cast<_CR>(__d.count()) * static_cast<_CR>(_CF::num) / static_cast<_CR>(_CF::den)));
            }
        };
        ...
        constexpr typename enable_if<__is_frequency<_ToFreq>::value, _ToFreq>::type frequency_cast(const frequency<_Rep, _Period>& __d)
        {
        ...
        template <typename _Rep, typename _Period> struct frequency
        {
            ...
            template <typename _Rep2,
                typename = typename enable_if<is_convertible<_Rep2, rep>::value &&
                    (treat_as_floating_point<rep>::value || !treat_as_floating_point<_Rep2>::value)>::type>
            ...
            template <typename _Rep2,
                typename _Period2,
                typename = typename enable_if<treat_as_floating_point<rep>::value ||
                    (ratio_divide<_Period2, period>::den == 1 && !treat_as_floating_point<_Rep2>::value)>::type>
        ...
        template <typename _Rep1, typename _Period1, typename _Rep2, typename _Period2>
        constexpr bool operator<(const duration<_Rep1, _Period1>& __lhs, const frequency<_Rep2, _Period2>& __rhs)
        {
        ...
        constexpr typename enable_if<__is_duration<_ToDur>::value, _ToDur>::type period_cast(const frequency<_Rep, _Period>& __d)
        {
    ...
    namespace literals
    {
        ...
        {
            constexpr std::chrono::hertz operator"" _hz(long double hz)
            {
                return std::chrono::hertz(hz);
            }

            constexpr std::chrono::hertz operator"" _hz(unsigned long long hz)
            {
                return std::chrono::hertz(hz);
            }
        }
