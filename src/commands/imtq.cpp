...
void StartActuationCurrent(uint16_t argc, char* argv[])
{
    ...
    const bool status = Main.imtq.driver.StartActuationCurrent(current, time);
    if (!status)
    {
...
void StartAllAxisSelfTest(uint16_t argc, char* argv[])
{
    UNREFERENCED_PARAMETER(argc);
    UNREFERENCED_PARAMETER(argv);
    const bool status = Main.imtq.driver.StartAllAxisSelfTest();
    if (!status)
    {
...
void StartBDotDetumbling(uint16_t argc, char* argv[])
{
    ...
    const bool status = Main.imtq.driver.StartBDotDetumbling(time);
    if (!status)
    {
...
void GetCalibratedMagnetometerData(uint16_t argc, char* argv[])
{
    ...
    const bool status = Main.imtq.driver.GetCalibratedMagnetometerData(result, newValue);
    if (!status)
    {
    ...
    if (!newValue)
    {
        Main.terminal.Printf("No new data\n");
        return;
    }
...
void GetSystemState(uint16_t argc, char* argv[])
{
    ...
    const bool status = Main.imtq.driver.GetSystemState(state);
    if (!status)
    {
...
void GetCoilData(uint16_t argc, char* argv[])
{
    ...
    UNREFERENCED_PARAMETER(argv);
    Vector3<Current> current;
    bool status = Main.imtq.driver.GetCoilCurrent(current);
    if (!status)
    {
    ...
    status = Main.imtq.driver.GetCoilTemperature(temp);
    if (!status)
    {
    ...
    Vector3<Dipole> dipole;
    status = Main.imtq.driver.GetCommandedActuationDipole(dipole);
    if (!status)
    {
        Main.terminal.Printf("GetCommandedActuationDipole failed!\n");
        return;
    }
...
void GetSelfTestResult(uint16_t argc, char* argv[])
{
    ...
    const bool status = Main.imtq.driver.GetSelfTestResult(result);
    if (!status)
    {
...
void GetHouseKeepingEngineering(uint16_t argc, char* argv[])
{
    ...
    bool status = Main.imtq.driver.GetHouseKeepingEngineering(result);
    if (!status)
    {
...
void GetParameter(uint16_t argc, char* argv[])
{
    ...
    bool status = Main.imtq.driver.GetParameter(id, paramData);
    if (!status)
    {
...
void ResetParameter(uint16_t argc, char* argv[])
{
    ...
    bool status = Main.imtq.driver.ResetParameterAndGetDefault(id, paramData);
    if (!status)
    {
