...
class FileSystemTelecommandsTest(BaseTest):
    ...
    def test_should_remove_file(self):
        ...
        self.system.comm.put_frame(telecommand.RemoveFile(respond_as=0x11, path=p))

        frame = self.system.comm.get_frame(20)

        self.assertEqual(frame.apid(), 0x11)
        self.assertEqual(frame.seq(), 0)
        self.assertEqual(frame.payload(), [0] + ensure_byte_list(p))
    ...
    def test_should_report_error_when_removing_non_existent_file(self):
        ...
        self.assertEqual(frame.apid(), 0x11)
        self.assertEqual(frame.seq(), 0)
        self.assertEqual(frame.payload(), [254] + ensure_byte_list(p))
