...
class DeepSleepBeacon(ResponseFrame):
    ...
    def decode(self):
        (self._time, self._raw_vbat_a, self._raw_vbat_b, self._reboot_to_normal) = struct.unpack(
            '<LHHL', ensure_string(self.payload()))
        self._vbat_a = epsa.BATCVoltage(self._raw_vbat_a)
        self._vbat_b = epsb.BATCVoltage(self._raw_vbat_b)

    def __str__(self):
        return '\tTime: {}s\n\t VBAT_A: {}\n\t VBAT_B: {}\n\t Reboot to normal: {}'.format(self._time/1000.0, self._vbat_a, self._vbat_b, self._reboot_to_normal)
