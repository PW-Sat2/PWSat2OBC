...
namespace
{
    ...
    TEST_F(EraseFlashExperimentTest, ShouldSendSuccessFrame)
    {
        ...
        EXPECT_CALL(_transmitter, SendFrame(IsDownlinkFrame(DownlinkAPID::Operation, 0, ElementsAre(0x67, 0, 2))));
    ...
    TEST_F(EraseFlashExperimentTest, ShouldSendErrorFrameOnChipEraseFailure)
    {
        ...
        EXPECT_CALL(
            _transmitter, SendFrame(IsDownlinkFrame(DownlinkAPID::Operation, 0, ElementsAre(0x67, 3, num(OperationResult::Failure)))));

        _exp.SetCorrelationId(0x67);
